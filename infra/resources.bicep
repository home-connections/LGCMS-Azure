...
var aggregatedAppSettings = union(
  ...
  reduce(cacheConnector.listConfigurations().configurations, {}, (cur, next) => union(cur, { '${next.name}': checkAndFormatSecrets(next) })), 
  {
     ...
     APP_KEY: '@Microsoft.KeyVault(SecretUri=https://lgcmspocmcfwe5qjx-vault.vault.azure.net/secrets/appKey/a273e398df5d4b8dabd89cc8e072dacb)' // Laravel encryption variable, required for Laravel to run.
